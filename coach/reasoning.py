...
TOOL_SCHEMAS = [
    ...
    {"name": "propose_block",
     "description": (
         ...
         "  KEEP THE COMP LIFT AS PRIMARY: the weak-point variation SUPPLEMENTS "
         "the competition lift — it never replaces it. Each trained lift MUST "
         "still have a primary session of the plain comp lift (e.g. a primary "
         "'Deadlift (top set)' day); put the variation (block pull, deficit, "
         "etc.) as that day's back-off or a separate secondary day. The engine "
         "rejects a lift trained ONLY through a variation. So for a deadlift "
         "lockout: keep the heavy primary Deadlift day AND add block pulls as "
         "its back-off / a secondary day — don't swap the deadlift day out for "
         "a block-pull day.\n"
    ...
    {"name": "propose_season",
     "description": (
         ...
         "train).\n\n"
         "MANDATORY WORKFLOW (do NOT skip): call compute_macrocycle_"
         "sizing(weeks_to_comp, experience, lifter_status) FIRST. It "
         "returns sequence[] with per-block planned_weeks (calendar "
         "weeks, which CAN VARY — e.g. for weeks_to_comp=19 it returns "
         "[5,5,4,5], not [5,5,5,5]). Use ITS sequence verbatim, "
         "converting planned_weeks → duration_weeks by SUBTRACTING 1 "
         "(productive weeks = calendar - 1 deload week) for each entry. "
         "Do NOT round all blocks to 4 productive weeks — that "
         "produces under-sized macrocycles that the engine REJECTS or "
         "flags with a calendar gap.\n\n"
         "  Worked example: compute_macrocycle_sizing(19) returns "
         "  [{volume, 5}, {strength, 5}, {strength, 4}, {peaking, 5}].\n"
         "  Your blocks_plan should be:\n"
         "    [{block_type: 'volume',   duration_weeks: 4, rationale: ...},\n"
         "     {block_type: 'strength', duration_weeks: 4, rationale: ...},\n"
         "     {block_type: 'strength', duration_weeks: 3, rationale: ...},\n"
         "     {block_type: 'peaking',  duration_weeks: 4, rationale: ...}]\n"
         "  Note the THIRD block has duration_weeks=3, not 4. That's "
         "  intentional — it absorbs the odd week so the chain fits "
         "  exactly.\n\n"
         "Each entry: ONLY block_type, duration_weeks, rationale. "
         "**DO NOT include outlines, exercises, weekly_plans, weights, "
         "or planned_start** — the engine backfills planned_start by "
         "walking backwards from competition_date so the LAST block's "
         "deload ends ON the meet.\n\n"
         "Engine feedback channels:\n"
         "  • too LONG (chain would start before today) → REJECTED with "
         "the correct weeks_to_comp number to use.\n"
         "  • too SHORT (calendar gap ≥ 3 weeks before the first block) "
         "→ REJECTED — re-propose using compute_macrocycle_sizing's "
         "exact sequence.\n"
         "  • mild gap (1-2 weeks): accepted with a `timing_note` field "
         "you should mention to the lifter ('you'll start a week from "
         "now; keep doing what you were doing until then').\n\n"
         "Future blocks are placeholders the UI shows as 'after volume, "
         "you'll do strength, then peaking'. The actual exercises and "
         "loads get built FRESH per block via propose_block when that "
         "block becomes active, adapting to how the previous block went."),
     "input_schema": {
         "type": "object",
         "properties": {
...
COACH_SYSTEM = (
    ...
    "RPE-anchored — loads come from the lifter's working 1RM, not from you):\n"
    "    - TOP SET (a single set, sets:1) climbs across the block to the "
    "block's terminal RPE on the FINAL week — strength/peaking -> RPE 9, "
    "volume -> RPE 8, technique -> RPE 7 — reps cycling down toward a single "
    "for strength/peaking. You do NOT set the weekly RPE; the engine ramps it "
    "(e.g. strength 4-week: RPE 6->7->8->9, reps 4->3->2->1).\n"
    "    - BACK-OFF (multiple sets, sets>1) progresses BY BLOCK TYPE: it "
    "CLIMBS week to week in volume/technique (the volume work is the driver — "
    "Sebastian adds load weekly, Bromley floats it up), and is held STATIC in "
    "strength/peaking (Ben: back-off load static while the top set drives).\n"
    "  So give each entry sensible sets/reps/intensity_pct/rpe_cap as a "
    "STARTING point only. intensity_pct is used for exercise SELECTION + "
    "validation and must sit in 0.55-0.94 (top set HIGHER than back-off). "
    "intensity_pct=0 on a main lift is a BUG — that encodes a non-barbell "
    "accessory (DB / cable / machine / pull-up / lateral raise). Squat / "
    "bench / deadlift and barbell variations (paused, tempo, close-grip, "
    "deficit, block pull, RDL) ALWAYS carry a real %TM. Never set a main-lift "
    "rpe_cap above 9.0.\n"
    "  STACK CONTEXT (Ben's BLOCK STACKING, 406rClFwNxY): when sequencing "
    "multiple blocks toward a comp, earlier blocks are sub-maximal DEPOSITS "
    "and the final block is the PR/peak. Reflect this in block SELECTION and "
    "SEQUENCING (call get_block_stacking_schedule), working backwards so the "
    "meet lands on the final week of the final block — the peaking deload is "
    "the taper.\n\n"
